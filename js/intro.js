...
const Intro = {
    ...
    bindEvents() {
        ...
        if (hearItModal) {
            ...
            if (backdrop) {
                backdrop.addEventListener('click', () => hearItModal.classList.add('hidden'));
            }
        }
        ...
        // Tap anywhere on intro screen to advance text steps immediately
        if (introSection) {
            introSection.addEventListener('click', (e) => {
                // Ignore clicks on buttons or active modals
    ...
    startSequence() {
        ...
        this.stepTimer2 = setTimeout(() => {
            if (this.currentStep === 2) this.advanceToStep3();
        }, 5600);
    },
    ...
    handleYesClick() {
        ...
        // Persist startDate permanently
        Storage.setStartDate(exactTimestamp);
        ...
        Storage.saveAchievementState('begin', true, new Date().toISOString());
