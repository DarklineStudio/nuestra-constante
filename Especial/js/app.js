...
const App = {
    async init() {
        ...
        // Fast local check to prevent intro screen flash on reload
        const fastStartDate = Storage.getStartDate();
        if (fastStartDate) {
            this.showMainDashboard();
        } else {
            this.showProposalScreen();
        }
    ...
    showMainDashboard() {
        ...
        try { Storage.ensureDataIntegrity(); } catch (e) {}
        ...
        const deliveryResetBox = document.getElementById('deliveryResetBox');
        ...
        // Primary Live Clock Counter Start
        try { if (window.Counter) Counter.start(); } catch (e) { console.warn('Counter err:', e); }

        // Secondary Modules safely initialized
        try { if (window.Timeline) { Timeline.init(); Timeline.render(); } } catch (e) { console.warn('Timeline err:', e); }
        try { if (window.Painting) { Painting.init(); } } catch (e) { console.warn('Painting err:', e); }
        try { if (window.Training) { Training.init(); Training.render(); } } catch (e) { console.warn('Training err:', e); }
        try { if (window.Capsule) { Capsule.init(); Capsule.render(); } } catch (e) { console.warn('Capsule err:', e); }
        try { if (window.Music) { Music.init(); } } catch (e) { console.warn('Music err:', e); }
        try { if (window.Letter) { Letter.init(); } } catch (e) { console.warn('Letter err:', e); }
        try { if (window.Vitals) { Vitals.startEcg(); } } catch (e) { console.warn('Vitals err:', e); }
    },

    showProposalScreen() {
        document.documentElement.classList.remove('has-start-date');
    ...
    switchTab(tabId) {
        ...
        if (tabId === 'historia' || tabId === 'timeline') {
            if (window.Timeline) { Timeline.init(); Timeline.render(); }
            ...
            if (window.Vitals) Vitals.startEcg();
            if (window.Training) Training.render();
            if (window.Capsule) Capsule.render();
        } else if (tabId === 'musica') {
            if (window.Music) { Music.init(); Music.render(); }
        }
    },
